...
function App() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    const total = (attackPercent !== 0 ? Number(attack) * (Number(attackPercent) / 100) : 0) +
      (defPercent !== 0 ? Number(def) * (Number(defPercent) / 100) : 0) +
      (speedPercent !== 0 ? Number(speed) * (Number(speedPercent) / 100) : 0)
